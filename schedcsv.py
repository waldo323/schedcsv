...
# separate the time and day into different variables
        if (field == "event_start"):
            ...
            slashdate = "{}/{}/{}".format(dashdate[6:7],
                                          dashdate[9:10], dashdate[0:4])
            session['startday'] = slashdate
            session["Start Date"] = session['startday']
            if fieldtext[-8:] != "nt_start":
                logging.debug(
                    fieldtext[len(fieldtext)-fieldtext.find(" ") + 2:-3])
                session['starttime'] = fieldtext[len(
                    fieldtext)-fieldtext.find(" ") + 2:-3]
...
alphabetical_rooms = {}
for room in rooms:
    alphabetical_rooms[room] = {'name': room,
...
presenterpacket = presenterpacket_template.render(
    agendas=speakersagendas, speakers=sorted(speakers))
# for x in sessions:
#    print x['bookname']
